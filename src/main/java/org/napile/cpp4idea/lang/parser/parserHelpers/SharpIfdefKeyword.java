...
import org.napile.cpp4idea.config.CConfiguration;
import org.napile.cpp4idea.lang.psi.CParserUtil;
...
public class SharpIfdefKeyword extends CommonParsing
{
	public static void parseIf(PsiBuilder builder)
	{
		...
		if(builder.getTokenType() == IDENTIFIER)
		{
			...
			active = elementType == S_IFDEF_KEYWORD ? CConfiguration.isDefined(builder.getTokenText()) : !CConfiguration.isDefined(builder.getTokenText());
		...
		while(!builder.eof())
		{
			CommonParsing.parseElement(builder, active ? CommonParsing.ACTIVE_BLOCK : 0);
